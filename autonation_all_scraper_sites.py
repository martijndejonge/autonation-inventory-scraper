...
BASE_PAYLOAD = {
    ...
    "preferences": {
        "pageSize": "18",
        "listing.config.id": "auto-new,auto-used",
...
def scrape_inventory(dealer_name, site_url, site_id):
    ...
    referer_url = f"{site_url.rstrip('/')}/all-inventory/index.htm"
    ...
    if all_vehicles:
        ...
        if total > 1000:
            ...
            for i in range(0, total, chunk_size):
                ...
                chunk_file = f"output/{safe_name}_all_inventory_part{part_num}.json"
                with open(chunk_file, "w", encoding="utf-8") as f:
                    json.dump(chunk, f, indent=2)
            print(f"✅ Saved {total} vehicles for {dealer_name} in {((total - 1) // 1000) + 1} chunks")
        else:
            file_path = f"output/{safe_name}_all_inventory.json"
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(all_vehicles, f, indent=2)
            print(f"✅ Saved {total} vehicles for {dealer_name}")
    else:
        print(f"⚠️ No vehicles found for {dealer_name}")
...
def main():
    ...
    for dealer in dealers:
        ...
        url = dealer["URL"]
        inventory_page = url.rstrip("/") + "/all-inventory/index.htm"
